...
public class CustomerServiceImpl implements CustomerService {
  ...
  public Customer getByEmail(String email) {
    return repository
        .findByEmail(email)
        .orElseThrow(
            () -> new CustomerNotFoundException(ApiError.ERROR_CUSTOMER_NOT_FOUND_BY_EMAIL));
  }
  ...
  public Customer getByEmailAndPassword(String email, String passwordHash) {
    return repository
        .findByEmailAndPassword(email, passwordHash)
        .orElseThrow(
            () ->
                new CustomerNotFoundException(
                    ApiError.ERROR_CUSTOMER_NOT_FOUND_BY_EMAIL_AND_PASSWORD_HASH));
  }
  ...
  public Customer addCustomer(Customer customer) {
    ...
    if (customer.getRoles() == null || customer.getRoles().isEmpty()) {
      roleRepository
          ...
          .ifPresent(role -> customer.setRoles(Arrays.asList(role)));
    }
