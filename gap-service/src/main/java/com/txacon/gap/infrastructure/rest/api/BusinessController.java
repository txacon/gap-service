...
public class BusinessController {
  ...
  public ResponseEntity<List<BusinessDTO>> getBusiness(Principal principal) {
    ...
    return ResponseEntity.ok(
        service.findByOwnerId(customerId).stream().map(mapper::toDTO).collect(Collectors.toList()));
  }
  ...
  public ResponseEntity<List<BusinessDTO>> getAllBusiness(Principal principal) {
    return ResponseEntity.ok(
        service.findAll().stream().map(mapper::toDTO).collect(Collectors.toList()));
  }
  ...
  public ResponseEntity<BusinessDTO> createBusiness(
      Principal principal, @RequestBody @NotNull BusinessDTO businessDTO) {
    Long customerId = Long.parseLong(principal.getName());
    return ResponseEntity.accepted()
        .body(mapper.toDTO(service.add(mapper.toDomain(businessDTO), customerId)));
  }
  ...
  public ResponseEntity<BusinessDTO> updateBusiness(
    ...
    Long customerId = Long.parseLong(principal.getName());
    return ResponseEntity.accepted()
        .body(mapper.toDTO(service.update(mapper.toDomain(businessDTO), customerId)));
  }
  ...
  @PreAuthorize("hasRole({'ROLE_SELLER'})")
  @GetMapping(
      value = "/{businessId}/products/{productId}",
  ...
  public ResponseEntity<ProductDTO> getBusinessProductById(
    ...
    Optional<ProductDTO> productSearch =
        mapper.toDTO(service.findById(businessId)).getProducts().stream()
            .filter(e -> e.getId().equals(productId))
            .findFirst();
    return ResponseEntity.ok(
        productSearch.orElseThrow(
            () -> new BusinessNotFoundException(ApiError.ERROR_PRODUCT_NOT_FOUND)));
  }

  @Loggable
  @PreAuthorize("hasRole({'ROLE_SELLER'})")
  @PostMapping(value = "/{businessId}/products", produces = MediaType.APPLICATION_JSON_VALUE)
  public ResponseEntity<Void> addBussinessProduct(
      @PathVariable @NotNull @Min(0) @Max(Long.MAX_VALUE) Long businessId,
  ...
  public ResponseEntity<Void> deleteBussinessProduct(
    ...
    boolean removed =
        mapper
            .toDTO(service.findById(businessId))
            .getProducts()
